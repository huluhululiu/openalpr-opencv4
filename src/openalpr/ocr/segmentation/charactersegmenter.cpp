...
//        }
  ...
  void CharacterSegmenter::removeSmallContours(vector<Mat> thresholds, float avgCharHeight,  TextLine textLine)
  {
    ...
    for (unsigned int i = 0; i < thresholds.size(); i++)
    {
      ...
      findContours(thresholdsCopy, contours, hierarchy, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE);
  ...
  //				    Point( 1, 1 ) );
      ...
      for (unsigned int j = 0; j < charRegions.size(); j++)
      {
        ...
        for (unsigned int c = 0; c < contours.size(); c++)
        {
          ...
          {
            // Erase this speckle
            drawContours(thresholds[i], contours, c, Scalar(0,0,0), cv::FILLED);

            if (this->config->debugCharSegmenter)
            {
              drawContours(imgDbgCleanStages[i], contours, c, COLOR_DEBUG_SPECKLES, cv::FILLED);
            }
          }
          else
          {
  ...
  void CharacterSegmenter::cleanBasedOnColor(vector<Mat> thresholds, Mat colorMask, vector<Rect> charRegions)
  {
    ...
    for (unsigned int i = 0; i < thresholds.size(); i++)
    {
      for (unsigned int j = 0; j < charRegions.size(); j++)
      {
        ...
        rectangle(boxChar, charRegions[j], Scalar(255,255,255), cv::FILLED);
        ...
        {
          rectangle(thresholds[i], charRegions[j], Scalar(0,0,0), cv::FILLED);

          if (this->config->debugCharSegmenter)
          {
            //vector<Mat> tmpDebug;
            //Mat thresholdCopy2 = Mat::zeros(thresholds[i].size(), CV_8U);
            //rectangle(thresholdCopy2, charRegions[j], Scalar(255,255,255), cv::FILLED);
  ...
  //		Mat tmpx = drawImageDashboard(tmpDebug, tmpytmp.type(), 1);
            ...
            circle(imgDbgCleanStages[i], topLeft, 5, COLOR_DEBUG_COLORFILTER, cv::FILLED);
          }
        }
      }
    }
  ...
  vector<Rect> CharacterSegmenter::filterMostlyEmptyBoxes(vector<Mat> thresholds, const vector<Rect> charRegions)
  {
    ...
    for (unsigned int i = 0; i < thresholds.size(); i++)
    {
      for (unsigned int j = 0; j < charRegions.size(); j++)
      {
        ...
        rectangle(tempImg, charRegions[j], Scalar(255,255,255), cv::FILLED);
  ...
  int CharacterSegmenter::isSkinnyLineInsideBox(Mat threshold, Rect box, vector<vector<Point> > contours, vector<Vec4i> hierarchy, float avgCharWidth, float avgCharHeight)
  {
    ...
    for (unsigned int i = 0; i < contours.size(); i++)
    {
      ...
      findContours(tempImg, subContours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
